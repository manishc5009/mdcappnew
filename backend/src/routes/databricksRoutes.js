...
router.post('/register-notebook', async (req, res) => {
  try {
    ...
    // Extract fields
    const { taskId, fileName, filesize, total_rows, selectedOption, selectedDataId, currentStep, uploadCompleted, id, username, email } = notebookData;
    // console.log('Extracted fields:', { taskId, fileName, selectedOption, selectedDataId, currentStep, uploadCompleted, email, username, id, filesize, total_rows });
...
// New endpoint to get notebooks for a user by userId
router.get('/user-notebooks/:userId', async (req, res) => {
  try {
...
router.delete('/notebook/:id', async (req, res) => {
  try {
    ...
    if (!notebookId) {
      return res.status(400).json({ error: 'Missing notebook id parameter' });
    }
