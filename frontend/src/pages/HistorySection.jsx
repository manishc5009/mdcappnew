...
const CACHE_KEY = 'historyNotebooksCache';
...
const HistorySection = () => {
  ...
  useEffect(() => {
    const fetchNotebooks = async () => {
      ...
      try {
        ...
        setNotebooks(cacheData.notebooks);
      } catch (err) {
  ...
  const handleResume = (notebook) => {
    // Navigate to upload page with notebook data in state for resuming
    navigate('/upload', { state: { resumeTask: notebook } });
  };
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {notebook.status === 1 ? (
                          <>
                            ...
                            <button className="text-gray-600 hover:text-gray-900 mr-3">Download</button>
                          </>
                        ) : (
                          <>
                            <button
                              className="text-green-600 hover:text-green-900 flex items-center mr-3"
                              onClick={() => handleResume(notebook)}
                              title="Resume"
                            >
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  const apiurl = import.meta.env.VITE_API_URL;
                                  const response = await fetch(`${apiurl}/api/databricks/notebook/${notebook.id}`, {
                                    method: 'DELETE',
                                  });
                                  if (!response.ok) {
                                    throw new Error('Failed to delete notebook');
                                  }
                                  // Remove deleted notebook from state
